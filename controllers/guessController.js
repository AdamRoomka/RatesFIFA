...
exports.createOrUpdateGuesses = async (req, res) => {
  ...
  if (req.body.guesses === undefined) {
    res
      .status(400)
      .json({
        message: "Guesses not defined",
        status: "fail",
        code: "GUESSES_NOT_DEFINED",
      });
    return;
  }
  ...
  req.body.guesses.forEach(async (guess) => {
    if (guess.matchId === undefined || guess.matchId === "") {
      res
        .status(400)
        .json({
          message: "MatchId not defined",
          status: "fail",
          code: "MATCHID_NOT_DEFINED",
        });
      return;
    }
    ...
    var matches = await matchModel
      .find({ type: match.type })
      .sort({ date: 1, time: 1 });

    var date = matches[0].date.split("-");
    var time = matches[0].time.split(":");
    const minutesForPlacingBetsBeforeFirstMatch =
      process.env.DISALLOW_USER_PLACE_GUESSES_MINUTES_BEFORE_FIRST_MATCH;
    var isFirstMatchStartsSoon =
      Date.now() -
        Date.parse(
          new Date(
            Date.UTC(date[0], date[1] - 1, date[2], time[0] - 3, time[1], 0)
          )
        ) /
          1000 <=
      minutesForPlacingBetsBeforeFirstMatch * 60000
        ? true
        : false;
    ...
    if (isFirstMatchStartsSoon) {
      res
        .status(400)
        .json({
          message: `Placing bets for matches in ${match.type} is not longer possible`,
          status: "fail",
          code: "WRONG_GUESS",
        });
      return;
    ...
    if (guess.score1 === undefined) {
      ...
      res
        .status(400)
        .json({
          message: "Score1 not defined",
          status: "fail",
          code: "SCORE_NOT_DEFINED",
        });
      return;
    }
    ...
    var existingGuess = await guessModel.findOne({
      userId: user._id,
      matchId: guess.matchId,
    });

    var newOrUpdatedGuess =
      existingGuess === null ? new guessModel() : existingGuess;
