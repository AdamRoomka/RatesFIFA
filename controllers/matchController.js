...
const {
  calculateMatchGuesses,
  calculateUserGainedPoints,
} = require("../scripts/matchGuessScoreCalculator");
...
exports.getAllMatches = async (req, res) => {
  try {
    ...
    for (var match of matches) {
      ...
      var matchGuesses = await guessModel
        .find({ matchId: match._id })
        .populate("userId");

      for (var guess of matchGuesses) {
        var guessObject = {
          userName: guess.userId.name,
          score1: guess.score1,
          score2: guess.score2,
          points: "",
        };
        ...
        if (match.completed) {
          var scoredPoints = calculateUserGainedPoints(
            match.score1,
            match.score2,
            guess.score1,
            guess.score2
...
exports.createMatch = async (req, res) => {
  ...
  if (team1 === null || team2 === null) {
    res
      .status(400)
      .json({
        message: "Wrong team1 or team2 ID",
        status: "fail",
        code: "WRONG_TEAM_ID",
      });
    return;
